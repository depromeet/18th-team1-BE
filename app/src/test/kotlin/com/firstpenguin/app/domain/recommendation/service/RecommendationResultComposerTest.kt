...
class RecommendationResultComposerTest {
    ...
    @Test
    fun `metadataScore가 낮고 semanticScore가 높아도 상위 3개 후보는 보존한다`() {
        ...
        assertTrue(
            quoteIds.indexOf(semanticDominatedQuoteId) in 0 until TOP_THREE_COUNT,
    ...
    @Test
    fun `상위 3개 밖의 metadataScore가 낮고 semanticScore가 높은 primary 후보는 top10 뒤로 미룬다`() {
        val semanticDominatedQuoteId = 1L
        val candidates =
            listOf(
                candidate(
