...
| 카테고리 | 도구 | 역할 |
|----------|------|------|
| 코드 스타일 | ktlint (kotlinter 플러그인) | Kotlin 공식 스타일 가이드 기반 포맷팅 강제 |
| 정적 분석 | detekt 2.0.0-alpha.1 | 코드 스멜, 복잡도, 잠재적 버그 탐지 |
...
- **Gradle 플러그인**: Kotlin 2.2 호환을 위해 `jlleitschuh` 대신 `kotlinter` 사용 (동일한 ktlint 엔진)
...
- **버전**: Kotlin 2.2 호환을 위해 2.0.0-alpha.1 사용. 빌드 도구이므로 alpha여도 프로덕션 코드에 영향 없음
...
```
PR 생성 시 (Fail Fast 원칙):
  lint job:
    1. lintKotlin       → 스타일 위반 시 실패
...
| 용도 | 명령어 |
|------|--------|
| 코드 스타일 검사 | `./gradlew lintKotlin` |
| 코드 스타일 자동 수정 | `./gradlew formatKotlin` |
| 정적 분석 | `./gradlew detekt` |
| 테스트 | `./gradlew test` |
| 커버리지 검증 | `./gradlew koverVerify` |
